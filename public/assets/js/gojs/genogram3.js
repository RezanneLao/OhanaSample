function initGenogram(data, clanIndividualData, user_id) {
    ...
    */
    ...
    myDiagram.linkTemplateMap.add("married",
        $(go.Link, {
                selectable: false,
                routing: go.Link.AvoidsNodes,
                curve: go.Link.JumpOver
    ...
    */
    ...
    setupDiagram(myDiagram, data, clanIndividualData, user_id);
}

function setupDiagram(diagram, array, clanIndividualData, focusId) {
    diagram.model = go.GraphObject.make(go.GraphLinksModel, {
        linkLabelKeysProperty: "labelKeys",
        nodeCategoryProperty: "gender",
        nodeDataArray: clanIndividualData
    });

    setupMaritalStatus(diagram, array);
    setupParents(diagram, array);
...
function findMarriage(diagram, a, b) {
    ...
    if (nodeA !== null && nodeB !== null) {
        ...
        while (it.next()) {
            ...
            if (link.data !== null && link.data.category === "married") return link;
...
function setupMaritalStatus(diagram, arrayData) {
    ...
    //         console.log(nodeDataArray[j].id);
    //         if(nodeDataArray[j].id === arrayData[i].id) {
    //             if(nodeDataArray[j].pid === arrayData[i].pid) {
    //                 if(arrayData[i].maritalStatus !== null) {
    //                     var connect = findMarriage(diagram, arrayData[i].pid, arrayData[i].id);
    //                     console.log(connect);
    //                     if (connect === null) {
    //                         if (arrayData[i].maritalStatus === "married") {
    ...
    for(var i = 0; i < arrayData.length; i++) {
        ...
        if(data.relationship === 'husband' || data.relationship === 'wife') {
            if(data.maritalStatus === 'married') {
                ...
                for(var j = 0; j < nodeDataArray.length; j++) {
                    console.log('sud')
                    if(data.id === nodeDataArray[j].id) userCon = nodeDataArray[j];
                    if(data.pid === nodeDataArray[j].pid) potentialCon = nodeDataArray[j];
                }
                ...
                console.log('pid', potentialCon)
            }
        }
    }
}

function setupParents(diagram, arrayData) {
    var model = diagram.model;
    var nodeDataArray = model.nodeDataArray;
